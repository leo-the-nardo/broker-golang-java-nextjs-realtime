package com.leothenardo.homebroker.wallets.entities;
...
public class Wallet {
	...
	private boolean validateIsAbleToSellAsset(String assetId, int shares) {
		...
		if (assetOnWallet.isEmpty() || assetOnWallet.get().getAvailableShares() < shares) {
			return false;
		}
		return true;
	}
	...
	public boolean initSell(String assetId, int shares) {
		...
		int indexToUpdate = getIndexAssetOnWalletById(assetId); // equals only compares symbol
	...
	public void computeSell(String assetId, int sharesSold, Double price) {
		...
		if (indexToUpdate == -1) {
			throw new IllegalStateException("Trying to buy an asset that does not belong to the wallet.");
		}
		var assetOnWallet = this.walletAssets.get(indexToUpdate);
